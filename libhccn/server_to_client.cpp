...
static size_t encode_ids (char* encoded, const std::optional<quint64>& session_id, const std::optional<quint64>& request_id, quint64 response_id)
{
    size_t off = 0;
...
static bool parse_meta (const QByteArray& data, qsizetype& off, bool& is_tail, bool& session_id_present, bool& request_id_present, quint64& fragment_number)
{
    if (data.size () < 1) {
...
Message::Message (
    ...
    const std::optional<quint64>& request_id,
    quint64 response_id,
...
QVector<QNetworkDatagram> Message::encode () const
{
    ...
    if (single_message_len <= 508) { // Single message
        ...
        encoded.append (encode_single_message_meta (session_id.has_value (), request_id.has_value ()));
...
QSharedPointer<Message> MessageFragmentCollector::build ()
{
    ...
    return QSharedPointer<Message> (new Message (head->host, head->port, head->session_id, head->request_id, head->response_id, data));
}
